...
export function ScreenSessionList({ dict: d, onSelectSession, onOpenSettings }: ScreenSessionListProps) {
  const [hoveredId, setHoveredId] = useState<string | null>(null)

  const groupLabel = (g: "today" | "yesterday") => (g === "today" ? d.today : d.yesterday)

  return (
    <div className="flex flex-col w-full max-w-[720px] rounded-2xl border border-[var(--glass-border)] bg-card overflow-hidden shadow-2xl shadow-black/50">
      {/* macOS title bar with search */}
      ...
      <div className="flex items-center justify-between px-6 pt-5 pb-1">
        ...
        <button className="flex items-center gap-2 rounded-full bg-primary px-5 py-2 text-sm font-semibold text-primary-foreground transition-opacity hover:opacity-90 active:opacity-80">
